    ...
    <Card className="overflow-hidden h-full flex flex-col">
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          <CardTitle className="text-lg">{ensemble.nom || 'Ensemble sans nom'}</CardTitle>
          {ensemble.email && (
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          {imageUrls.length > 0 ? (
            imageUrls.map((url, index) => (
              <div 
                key={index}
                className={`bg-cover bg-center rounded-md ${index === 0 ? 'col-span-2 h-32' : 'h-20'}`}
                style={{ backgroundImage: `url(${url})` }}
              />
            ))
          ) : (
            <div className="col-span-2 h-32 bg-muted rounded-md flex items-center justify-center">
